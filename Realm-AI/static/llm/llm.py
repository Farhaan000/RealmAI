...
"""
    ...
    def lim_llm_invoker(self, prompt: str):
        ...
        # Format the template with the query and current `self.human_ai_history_lim`
        formatted_template = self.template.format(context=context, history=self.human_ai_history_lim, question=prompt)
        ...
        if self.human_ai_history_lim:
            self.human_ai_history_lim += f"\n{new_history}"
        else:
            self.human_ai_history_lim = new_history
